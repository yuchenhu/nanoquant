...
> **定位**：本项目实际接入的 **26 个 tushare 接口**的权威使用说明。
> 每个接口的更新策略都有三重背书：① **官方文档依据**（参数语义 + 更新频率）；② **MCP 实测**（用历史数据验证能取到数、不穿越）；③ **主键无重复的数据论证**（实测行数统计）。
> 最后验证：2026-06-21 ｜ 配套代码：`config/tushare_apis.json` + `data/etl/loader.py` + `pipeline/incremental/*` + `config/database.py`
...
| 26 | dividend | dividend | dividend | by_ex_date | overwrite | ex_date | ts_code+end_date+ann_date+div_proc+update_flag | 实时 |
...
### 2.2 by_trade_date（overwrite / trade_date）— 行情类（15 个）
...
### 5.3 dividend PK = `ts_code+end_date+ann_date+div_proc+update_flag`
