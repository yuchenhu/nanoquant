...
> **定位**：本项目实际接入的 **31 个 tushare 接口**的权威使用说明。
> 每个接口的更新策略都有三重背书：① **官方文档依据**（参数语义 + 更新频率）；② **MCP 实测**（用历史数据验证能取到数、不穿越）；③ **主键无重复的数据论证**（实测行数统计）。
> 最后验证：2026-07-04 ｜ 配套代码：`config/tushare_apis.json` + `data/etl/loader.py` + `pipeline/incremental/*` + `config/database.py`
...
| 22 | fund_factor_pro | fund_factor_pro | fund_factor_pro | by_trade_date | overwrite | trade_date | ts_code+trade_date | 每日 |
| 23 | fund_nav | fund_nav | fund_nav | by_trade_date | overwrite | nav_date | ts_code+nav_date | 每日 |
| 24 | income_vip | income_vip | income | by_period | overwrite | end_date | ts_code+end_date+ann_date+f_ann_date+update_flag | 实时(随财报) |
| 25 | balancesheet_vip | balancesheet_vip | balancesheet | by_period | overwrite | end_date | 同 income（5 列） | 实时 |
| 26 | cashflow_vip | cashflow_vip | cashflow | by_period | overwrite | end_date | 同 income（5 列） | 实时 |
| 27 | disclosure_date | disclosure_date | disclosure_date | by_period | overwrite | end_date | ts_code+end_date | 不定期 |
| 28 | dividend | dividend | dividend | by_ex_date | overwrite | ex_date | ts_code+end_date+ex_date+div_proc+update_flag | 实时 |
| 29 | moneyflow_hsgt | moneyflow_hsgt | moneyflow_hsgt | by_trade_date | overwrite | trade_date | trade_date | 每日盘后 |
| 30 | margin | margin | margin | by_trade_date | overwrite | trade_date | trade_date+exchange_id | 次日 9h |
| 31 | limit_list_d | limit_list_d | limit_list_d | by_trade_date | overwrite | trade_date | trade_date+ts_code | 每日盘后 |
...
| fund_factor_pro | 场内基金技术面因子（60+ 指标：MA/MACD/RSI/Boll/KDJ/ATR，ETF 轮动截面排序信号源） |
| fund_nav | 公募基金净值（日频 unit_nav，用于 ETF 折溢价率 = close/nav - 1） |
...
- `fund_factor_pro`(ts_code=510050.SH, start_date=20260601)：全量 80+ 技术指标列（MA/MACD/RSI/Boll/KDJ/ATR 等），Tushare 自产、覆盖全历史。**积分要求 5000**。列名统一带 `_bfq` 后缀（不复权），ETF 除权影响极小可直接用。
- `fund_nav`(ts_code=510300.SH, market=E, start_date=20260601)：返回 `unit_nav/accum_nav/adj_nav/accum_div/update_flag`，按 `nav_date`（净值日期）组织。**注意**：`nav_date` 是净值计算日，和交易日期大部分对齐但节假日无值。biz_date_col='nav_date'，partition_col='nav_date'，取数时把调度传的 `trade_date` 映射为 `nav_date`。

**ETF 持仓穿透风控**：用已有 `index_weight` 穿透（红利低波等指数实测均收录），**不需要 fund_portfolio**。

### 4.6 moneyflow_hsgt — 沪深港通资金流向 ⚠️ 单位跳变

**官方文档**（[doc_id=47](https://tushare.pro/document/2?doc_id=47)）：
- 字段：`trade_date`, `ggt_ss`, `ggt_sz`, `hgt`, `sgt`, `north_money`, `south_money`
- 更新策略：by_trade_date，按交易日增量

**⚠️ 已确认坑：`north_money` 单位在 2024-08-19 发生跳变（万元→元，×10000）**

| 时段 | 单位 | 日值范围 | 识别方式 |
|------|------|---------|---------|
| 2014-01 ~ 2024-08-18 | 万元 | -17,000 ~ +21,000 | trade_date < "2024-08-19" |
| 2024-08-19 至今 | 元 | -13,000 ~ +510,000 | trade_date >= "2024-08-19" |

- 跳变精确日：2024-08-19（前一天 `-6,774` 万元，当天 `88,110` 元）
- 修复位置：`market_sentiment_monthly.py::_normalize_north_money()`，自动归一到万元
- 验证脚本：`scripts/find_north_money_break.py`
- 详见 DEV_GUIDE.md §7.16

---

### 4.7 fund_basic — 公募基金基本信息

**官方文档**（[doc_id=19](https://tushare.pro/document/2?doc_id=19)）：
- 字段：`ts_code`, `name`, `management`, `fund_type`, `invest_type`, `list_date`, `benchmark`, `status`, `m_fee`, `c_fee`, `market`
- 更新策略：full_refresh，每次全量拉取（基金信息变更低频）

**关键字段说明**：
| 字段 | 含义 | 用途 |
|------|------|------|
| `fund_type` | 基金类型（股票型/债券型/货币型/REITs） | 策略过滤 |
| `invest_type` | 投资类型（被动指数型/增强指数型） | 区分 ETF vs 主动基金 |
| `management` | **基金公司名**（非管理费率！管理费看 `m_fee`） | ⚠️ 坑：列名误导 |
| `benchmark` | 业绩基准（混合公式，如"沪深300×70%+中证全债×30%"） | ❌ 无法机械解析 |
| `name` | 基金名称（如"华泰柏瑞沪深300ETF"） | 用作 `extract_index_name()` 输入 |

**⚠️ `benchmark` 列无法直接解析跟踪指数**：返回的是混合加权基准公式，不是单一指数代码。ETF 的跟踪指数需要从 `name` 中用 `config/etf_universe.extract_index_name()` 正则提取。

---

### 4.8 fund_daily — 公募基金日行情

**官方文档**（[doc_id=19](https://tushare.pro/document/2?doc_id=19)）：
- 字段：`ts_code`, `trade_date`, `open/high/low/close`, `pre_close`, `change`, `pct_chg`, `vol`, `amount`
- 更新策略：by_trade_date，按交易日增量
- 说明：所有场内场外基金（含 ETF/LOF/封闭式），`trade_date` 为 `yyyy-mm-dd` 字符串
...
- 更新策略：by_trade_date
- 说明：ETF 有真实分红拆分→复权因子≠1（与股票复权逻辑相同）。**场外基金 adj_factor 恒为 1**

---

### 4.10 fund_nav — 基金净值（日频）

**官方文档**（[doc_id=19](https://tushare.pro/document/2?doc_id=19)）：
- 字段：`ts_code`, `nav_date`, `unit_nav`（单位净值）, `accum_nav`（累计净值）, `adj_nav`（复权净值）, `accum_div`, `update_flag`
- 更新策略：by_trade_date（`biz_date_col='nav_date'`）
- **⚠️ 不每日更新**：净值通常每周 3-5 次，节假日无值。`panel_fund_daily` 用 `merge_asof backward` 取最近净值，不穿越

---

### 4.11 fund_share — 基金份额

**官方文档**（[doc_id=19](https://tushare.pro/document/2?doc_id=19)）：
- 字段：`ts_code`, `trade_date`, `fd_share`（总份额，万份）
- 更新策略：by_trade_date
- **⚠️ 月频居多**：份额不是每日公布，大部分基金月度更新。`panel_fund_daily` 用 `merge_asof backward` 取最近公布值
- **⚠️ 含场外基金**：代码以 `.OF` 结尾的是场外基金，场内 ETF 以 `.SH/.SZ` 结尾
