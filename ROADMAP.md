...
## 2026-07-22 进度：write_mode upsert 全清零 + 2010 backfill 验证 + 双轨制废弃

- [x] **`_lint_consistency.py` lint 脚本落地**（N5.1 完成）：3 类 check（schedule_compute 依赖一致性 / upsert 残留 / 固定阈值），tokenize 过滤注释，err 阻塞 warn 不阻塞
- [x] **13 处 upsert 残留全部清零**（N5.5 完成）：
  - 基类默认值 `upsert → overwrite`（`core/calculator.py`）
  - 6 factor + 1 label + 5 panel calculator 各加 `partition_col`（9 个 trade_date + 3 个 snapshot_date）
  - 删除 `signals/generator.py`（signal_rebalance 接口废弃，write_mode=upsert 违反项目规则）
  - 清理 `signals/__init__.py` / `scripts/run_strategy.py` / `tests/test_step10_strategy.py`（skip）
...
## 历史进度归档（阶段 0 / 0.5 已完成项）

> 以下为 2026-06 至 2026-07-08 的完成项归档，保留作为上下文。
