...
| **验证**（必过门，按序） | ① **mock 跑通**（语法对）<br>② **单日回补**（连库 1 天，看列对齐/null/行数）<br>③ **区间回补**（1 个月）<br>④ **全量回补**<br>⑤ **EDA**：趋势与 A 股历史印证；`inf/NaN` 处理（`_safe_div` 包装除零）；单位跳变扫描。<br>⑥ `merge_asof + by` 陷阱：**只按 `on` 列排序**（全局单调），不按 `by+on`（跨 ts_code 重置会报 `left keys must be sorted`）。<br>⑦ 指标列必须注释**物理意义**（回答什么具体问题 + 公式来源），不写"XX指标"废话。 |
