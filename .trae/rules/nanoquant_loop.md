...
| H13 | **API 变更同步 README** | `--only/--solo/--start/--layer` 等高频 CLI 参数、常用补数命令、典型工作流——任何变更后**必须**同步更新 `README.md` 的对应章节。README 是给人看的 API 文档，过期=误导。 |
...
| **持久** | 信号落库表名待定（原 `signal_rebalance` 接口已废弃删除，后续如需调仓信号在 portfolio/ 层重写）。 |
| **调度** | 周频调仓。 |
