...
class LabelCalculator(BaseCalculator):
    """Label 计算层基类。

    - table_name 不含 label_ 前缀（本类自动加）
    ...
    # 前缀（label 表统一加，与 panel / factor / 接入层表区分）
    TABLE_PREFIX: str = "label_"
    ...
    def process_data(self, data: pd.DataFrame, **params: Any) -> pd.DataFrame:
        ...
        接收 get_data 的输出，做标签计算/转换。update 会透传 start_date/end_date。
        """
